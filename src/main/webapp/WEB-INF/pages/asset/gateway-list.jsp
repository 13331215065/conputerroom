...
<![endif]-->
	...
	<body>
		...
		<div class="page-container">
			...
			<a  class="btn btn-success" id="" name=""  href="${basePath }/asset/gogatewayAdd.do"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
				</form>
			</div>
					...
					<tbody>
					 <c:forEach items="${gateway }" var="gw">
						   <tr class="text-c">
							...
							<td>${gw.id }</td>
							<td>${gw.cconfig.gateName }</td>
							<td>${gw.cconfig.config }</td>
							<td>${gw.inTime }</td>
							<c:if test="${gw.repairState==0}">
                                   <td>正常</td>
                                 </c:if>
                                 <c:if test="${gw.repairState==1}">
                                   <td>维修</td>
                                 </c:if>
                                 <c:if test="${gw.repairState==2}">
                                   <td>报废</td>
                                 </c:if>			
							<td class="f-14 product-brand-manage">
								<a style="text-decoration:none" href="${basePath }/asset/gogatewayedit.do?id=${gw.id}" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="del(${gw.id},${gw.cconfig.id })" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
		...
		<script type="text/javascript">
		    function del(id,configId){
		    	if(confirm("您确定要删除吗?")){
		    		window.location.href="${basePath}/asset/delasset.do?id="+id+"&configId="+configId;
		    	}
		    }
		</script>
		<!-- <script type="text/javascript">
		...
		<script type="text/javascript">
		 /*  var gateway;
			$.ajax({
				url:"${basePath}/asset/getGatewayData.do",
				type:"post",
				dataType:"json",
				async:false,
