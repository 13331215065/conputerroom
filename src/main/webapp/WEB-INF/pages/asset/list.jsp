...
<![endif]-->
	...
	<body>
		...
		<div class="page-container">
			...
			</span>--><a  class="btn btn-success" id="" name=""  href="${basePath }/asset/goAdd.do"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
				</form>
			</div>
			...
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-sort">
					...
					<template v-for="asset in assets">
						<tr class="text-c">
							<td><input class="check" id="check" name="items[]"
											:value="asset.id" type="checkbox" onclick="setAll()"></td>
							<td>{{asset.id}}</td>
							<td>{{asset.name}}</td>
							<td>{{asset.assetType}}</td>
							<td>{{asset.comtract}}</td>
							<td>{{asset.computerroom}}</td>
							<td class="text-l">{{asset.inTime}}</td>
							<td class="text-l">{{asset.repairState}}</td>
							<td class="text-l">{{asset.phone}}</td>
