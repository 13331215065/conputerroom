...
<![endif]-->
	...
	<body>
		...
		<div class="page-container">
			...
			<a  class="btn btn-success" id="" name=""  href="${basePath }/asset/gostorageAdd.do"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
				</form>
			</div>
			...
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-sort">
					...
					<tbody>
					 <template v-for="storage in storages">
						   <tr class="text-c">
						     <td width=3%><input type="checkbox" name="" value="" ></td>
							<td>{{storage.id}}</td>
							<td>{{storage.name}}</td>
							<td>{{storage.storageConfig.config }}</td>
							<td>{{storage.inTime }}</td>
                            <td v-if="storage.repairState==0">正常</td>
                            <td v-if="storage.repairState==1">维修</td>
                            
							<td class="f-14 product-brand-manage">
								<a style="text-decoration:none" :href="'${basePath }/asset/gostorageEdit.do?id='+storage.id" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" :onclick="'del('+storage.id+')'" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
		...
		<script type="text/javascript">
		    function del(id){
		    	if(confirm("您确定要删除吗？")){
		    		window.location.href="${basePath}/asset/delstorageConfig.do?id="+id;
		    	}
		    }
		</script>
		<!-- <script type="text/javascript">
	...
	<script type="text/javascript">
			var storages;	
			$.ajax({
				  	url:"${basePath}/asset/getStorageData.do",
			    	dataType:"json",
			    	async:false,
